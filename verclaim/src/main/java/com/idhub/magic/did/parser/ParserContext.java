...
public class ParserContext {
	...
	public void push(String rulename, String trace) {
		...
		if (traceOn) {
			...
			System.out.println(
					index + ": " + text.substring(index, index + 10 > text.length() ? text.length() : index + 10)
							.replaceAll("[\\x00-\\x1F]", " "));
		}
	}
	...
	public void pop(String function, boolean result) {
		...
		if (traceOn) {
			System.out.println("<- " + level-- + ": " + function + "(" + (result ? "true" : "false") + ",s=" + start
					+ ",l=" + (index - start) + ",e=" + errorIndex + ")");
		}
