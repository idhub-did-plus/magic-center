package com.idhub.magic.ldsignatures.crypto.impl;
...
import com.idhub.magic.ldsignatures.crypto.RandomProvider;
import com.idhub.magic.ldsignatures.crypto.SHA256Provider;
...
public class NaClSodiumEC25519Provider extends EC25519Provider {
	...
	public void generateEC25519KeyPair(byte[] publicKey, byte[] privateKey) throws GeneralSecurityException {
		...
		if (privateKey.length != Sodium.CRYPTO_SIGN_ED25519_SECRETKEYBYTES)
			throw new GeneralSecurityException("Invalid private key length.");
		if (publicKey.length != Sodium.CRYPTO_SIGN_ED25519_PUBLICKEYBYTES)
			throw new GeneralSecurityException("Invalid public key length.");
		...
		System.arraycopy(publicKey, 0, privateKey, Sodium.CRYPTO_SIGN_ED25519_PUBLICKEYBYTES,
				Sodium.CRYPTO_SIGN_ED25519_PUBLICKEYBYTES);
	}
	...
	public byte[] sign(byte[] message, byte[] privateKey) throws GeneralSecurityException {
		...
		if (ret != 0)
			throw new GeneralSecurityException("Signing error: " + ret);
	...
	public boolean validate(byte[] message, byte[] signatureValue, byte[] publicKey) throws GeneralSecurityException {
		...
		if (signatureValue.length != Sodium.CRYPTO_SIGN_ED25519_BYTES)
