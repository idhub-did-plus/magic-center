package com.idhub.magic.ldsignatures.crypto;
...
public abstract class EC25519Provider {
			...
			synchronized (EC25519Provider.class) {
				...
				if (result == null) {
					...
					ServiceLoader<EC25519Provider> serviceLoader = ServiceLoader.load(EC25519Provider.class,
							EC25519Provider.class.getClassLoader());
					Iterator<EC25519Provider> iterator = serviceLoader.iterator();
					if (!iterator.hasNext())
						throw new RuntimeException("No " + EC25519Provider.class.getName() + " registered");
	...
	public abstract void generateEC25519KeyPairFromSeed(byte[] publicKey, byte[] privateKey, byte[] seed)
			throws GeneralSecurityException;

	public abstract byte[] sign(byte[] message, byte[] privateKey) throws GeneralSecurityException;

	public abstract boolean validate(byte[] message, byte[] signatureValue, byte[] publicKey)
			throws GeneralSecurityException;
}
